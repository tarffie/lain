...
class Die {
   ...
   *
   * @throws {Error} If `sides` is undefined or if `qty` is not a positive integer
   */
  constructor(
    sides: number,
    quantity: number,
    ...
    assert(quantity > 0, "'quantity' is required and must be a positive integer.")
    this.sides = sides;
   ...
   * @returns {Promise<number>} The result of the roll, including modifiers.
   */
  ...
  async rollDice(die: Die): Promise<number> {
    const results = new Array(die.quantity).fill(0).map(() => this.roll(die.sides, die.modifier));
